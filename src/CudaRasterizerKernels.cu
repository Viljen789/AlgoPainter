...
__device__ inline void blendPixels_device(Pixel& background, const Pixel& foreground) {
    ...
    background.r = static_cast<uint8_t>(
        (static_cast<unsigned int>(background.r) * invAlpha + static_cast<unsigned int>(foreground.r) * alpha) >> 8);
    background.g = static_cast<uint8_t>(
        (static_cast<unsigned int>(background.g) * invAlpha + static_cast<unsigned int>(foreground.g) * alpha) >> 8);
    background.b = static_cast<uint8_t>(
        (static_cast<unsigned int>(background.b) * invAlpha + static_cast<unsigned int>(foreground.b) * alpha) >> 8);
}
...
__global__ void renderKernel_perPixel(const CudaGene* population, Pixel* renderedBuffers, unsigned int numIndividuals,
                                      unsigned int genesPerIndividual, unsigned int imgWidth, unsigned int imgHeight) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    unsigned int individualIdx = blockIdx.z;

    if (x >= imgWidth || y >= imgHeight || individualIdx >= numIndividuals)
        return;
    ...
    for (unsigned int i = 0; i < genesPerIndividual; ++i) {
        ...
        for (int s = 0; s < 4; ++s) {
            ...
            if (gene.type == SHAPE_CIRCLE) {
                if (lx * lx + ly * ly <= gene.sizeX * gene.sizeX)
                    inside = true;
            } else if (gene.type == SHAPE_RECTANGLE) {
                if (fabsf(lx) <= gene.sizeX * 0.5f && fabsf(ly) <= gene.sizeY * 0.5f)
                    inside = true;
                ...
                if (is_inside_triangle(lx, ly, x0, y0, x1, y1, x2, y2))
                    inside = true;
            }

            if (inside)
                blendPixels_device(samples[s], geneColor);
        }
    }
    ...
    renderedBuffers[pixelIdx] = {
        ...
        static_cast<uint8_t>((static_cast<int>(samples[0].b) + samples[1].b + samples[2].b + samples[3].b) >> 2), 255};
}
...
__global__ void fitnessKernel(const Pixel* renderedBuffers, const Pixel* targetImage, float* fitnessResults,
...
__global__ void clearBuffersKernel(Pixel* renderedBuffers, unsigned int numIndividuals, unsigned int imgWidth,
    ...
    if (idx < totalPixelsAcrossBuffers)
        renderedBuffers[idx] = clearColor;
}

void CudaRasterizer::launchRenderKernel(CudaGene* d_population, Pixel* d_renderedBuffers, unsigned int populationSize,
    ...
    renderKernel_perPixel<<<blocks, threads>>>(d_population, d_renderedBuffers, populationSize, genesPerIndividual,
                                               width, height);
}

void CudaRasterizer::launchFitnessKernel(Pixel* d_renderedBuffers, Pixel* d_targetImage, float* d_fitnessResults,
                                         unsigned int populationSize, unsigned int width, unsigned int height) {
    fitnessKernel<<<populationSize, 256, 256 * sizeof(float)>>>(d_renderedBuffers, d_targetImage, d_fitnessResults,
                                                                populationSize, width, height);
}

void CudaRasterizer::launchClearBuffersKernel(Pixel* d_renderedBuffers, unsigned int populationSize, unsigned int width,
