...
bool mutateIndividual(Individual& I, std::mt19937& rng, unsigned canvasW, unsigned canvasH, float mutationRate) {
    ...
    for (auto& gene : I) {
        if (prob(rng) < mutationRate) {
            ...
            switch (type) {
                case 0:
                    pos.x = std::clamp(pos.x + posDelta(rng), 0.0f, (float)canvasW);
                    break;
                case 1:
                    pos.y = std::clamp(pos.y + posDelta(rng), 0.0f, (float)canvasH);
                    break;
                case 2:
                    size.x = std::clamp(size.x + sizeDelta(rng), 2.0f, 200.0f);
                    break;
                case 3:
                    size.y = std::clamp(size.y + sizeDelta(rng), 2.0f, 200.0f);
                    break;
                case 4:
                    rot = std::fmod(rot + rotDelta(rng) + 360.0f, 360.0f);
                    break;
                case 5:
                    col.r = std::clamp((int)col.r + colDelta(rng), 0, 255);
                    break;
                case 6:
                    col.g = std::clamp((int)col.g + colDelta(rng), 0, 255);
                    break;
                case 7:
                    col.b = std::clamp((int)col.b + colDelta(rng), 0, 255);
                    break;
                case 8:
                    col.a = std::clamp((int)col.a + colDelta(rng), 10, 255);
                    break;
